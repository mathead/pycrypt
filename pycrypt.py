...
class Decoder ():
	...
	def getScores (self, s, freqs, key, iterations=0, score_list=False):
		...
		if (score_list):
			...
			return [scores[0] / 60, scores[1] / 2, scores[2] / 1.5]
...
b = d.generateKey(lipsum, iterations=500, mutations=40, population=40, log=False)
# run("d.generateKey(lipsum, iterations=1, mutations=0, population=1000, log=True)")
for a in b:
