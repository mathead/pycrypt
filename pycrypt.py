...
class Decoder ():
	...
	def __init__ (self, frequency=ENGLISH_FREQUENCY, frequencies=None):
		...
		if (frequencies == None):
			self.frequencies = [self.baseFrequency, self.ENGLISH_BIGRAMS]
	...
	def getScoreNgrams (self, s, freqs, key, score_list=False):
		...
		if (score_list):
			return scores
		return sum(map(lambda x: (x[0] + 1) * x[1], enumerate(scores))) / sum(range(len(scores)))
	...
	def generateKey (self, s, population=10, mutations=10, perm_func=DEF_PERM_FUNC, iterations=900, cur=None, log=False, f=None):
		...
		if (f == None):
			f = [self.getFrequencies(s, 1), self.getFrequencies(s, 2)]
		...
		scores = []
		for m in mutants:
			scores.append(self.getScoreNgrams(s, f, m))

		mutants = sorted(zip(scores, mutants))[-10:]
		cur = map(lambda x: x[1], mutants)
		if (log):
...
# print d.getScoreNgrams(lipsum, [d.getFrequencies(lipsum, 1), d.getFrequencies(lipsum, 2)], dict(zip(d.alphabet, d.alphabet)))

b = d.generateKey(lipsum, iterations=20, log=True)
for a in b:
	print d.getScoreNgrams(lipsum, [d.getFrequencies(lipsum, 1), d.getFrequencies(lipsum, 2)], a)
